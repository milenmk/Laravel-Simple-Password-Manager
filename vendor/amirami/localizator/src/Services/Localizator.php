...
class Localizator
{
    ...
    protected function collect(Translatable $keys, string $type, string $locale, bool $removeMissing): Translatable
    {
        ...
        return $keys
            ->merge(
                $this->getCollector($type)->getTranslated($locale)
                    ->when(
                        $removeMissing, function (Translatable $keyCollection) use ($keys) {

                        return $keyCollection->intersectByKeys($keys);
                    }
                    )
            )->when(
                config('localizator.sort'), function (Translatable $keyCollection) {
